...
function moveToCenter()
{
    ...
    {
        OMAR.imageManipulator.moveToCenter();
    }
}
...
function convertPathAreaMetersToTargetUnit( geodLength, pathLength, area, targetUnit )
{
    ...
    area = Math.round( targetArea * inchesSourceArea * precision ) / precision;
...
function rotateTextFieldChange( angle )
{
    if ( rotateSlider )
    {
        ...
        rotateSlider.subscribe( "change", function ()
        {
...
function zoomMaxExtentClicked()
{
    ...
    if ( rasterEntry.numberOfResLevels > 1 )
    {
...
function checkOverview()
{
    ...
    var request = OpenLayers.Request.POST( {
        url: url + "?id=" + rasterEntry.id + "&max=1&format=json&fields=numberOfResLevels&labels=rlevels&_=" + new Date().getTime(),
...
function chipImage( format )
{
    ...
    document.location.href = links.getTile + "?" + params.toUrlParams();
}

function shareImage()
{
    var center = OMAR.imageManipulator.getCenterLocal();
    var baseURL = links.baseUrlAbsolute;
...
function exportTemplate()
{
    ...
    var northArrowAngle = parseFloat( $( "rotateAngle" ).value ) - rasterEntry.azimuthAngle;
    ...
    var y = Math.round( centerView.y - h / 2 );
    var z = this.map.getZoom();
    var getTileParams = new OmarImageSpaceGetTileParams();
    getTileParams.setProperties( document );
    getTileParams.setProperties(
        {
            'x': x,
            'y': y,
            'scale': scale,
    ...
    var imageUrl = links.getTileAbsolute + "?" + getTileParams.toUrlParams();
    ...
    $( "footerSecurityClassificationTextFormInput" ).value = "";
    $( "headerDescriptionTextFormInput" ).value = "Country: " + countryCode;
    $( "headerSecurityClassificationTextFormInput" ).value = "UNCLASS";
    $( "headerTitleTextFormInput" ).value = imageId;
    $( "imageUrlFormInput" ).value = imageUrl;
    $( "northAngleFormInput" ).value = northArrowAngle;
...
function init( mapWidth, mapHeight )
{
    ...
    pqePoint.displayUnit = pqeDisplayUnit;
    ...
    OMAR.imageManipulator.northAngle = rasterEntry.azimuthAngle;
    OMAR.imageManipulator.upIsUpAngle = upIsUpRotation;
    ...
    format = "image/jpeg";
    resLevels = rasterEntry.numberOfResLevels;
    initFlag = 1;
    ...
    {
        this.setValue( Math.ceil( (value % 360.0) / 2 ) );
    };
    rotateSlider.subscribe( "change", function ()
    {
        sliderRotate( this.getRealValue() );
