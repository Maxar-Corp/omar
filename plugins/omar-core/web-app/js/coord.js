function CoordinateConversion()
{
	...
	var dmsUseSpaces = false;
	
	this.ddToDms = function(lat, lon) {
		latDegAbs = Math.abs(lat);
		latDeg = Math.floor(latDegAbs);
		
		latMinAbs = Math.abs((latDegAbs - latDeg) * 60);
		latMin = Math.floor(latMinAbs);
		
		latSec = Math.abs((latMinAbs - latMin) * 60);
		latSec = latSec.toFixed(2);
		...
		if(dmsUseSpaces == true) {
			latDms = latDeg + " " + latMin + " " + latSec + " " + latOri;
		}
		else {
			latDms = latDeg + "" + latMin + "" + latSec + "" + latOri;
		}
		
		lonDegAbs = Math.abs(lon);
		lonDeg = Math.floor(lonDegAbs);
		
		lonMinAbs = Math.abs((lonDegAbs - lonDeg) * 60);
		lonMin = Math.floor(lonMinAbs);
		
		lonSec = Math.abs((lonMinAbs - lonMin) * 60);
		...
		if(dmsUseSpaces == true) {
			lonDms = lonDeg + " " + lonMin + " " + lonSec + " " + lonOri;
		}
		else {
			lonDms = lonDeg + "" + lonMin + "" + lonSec + "" + lonOri;
		}
	...
	{
		dd = Math.abs(deg) + Math.abs(min / 60) + Math.abs(sec / 3600);
		
		if(ori == "S" || ori == "s" || ori == "W" || ori == "w") {
			dd = -dd;
		}
