...
class WMSRequest
{
  ...
  def toMap()
  {
     return [bbox: bbox, width: width as Integer, height: height as Integer, format: format, layers: layers, srs: srs, service: service,
			...
			interpolation:interpolation,time: time, null_flip: null_flip, bands:bands, exception: exception, filter:filter, 
			quicklook: quicklook].sort { it.key }
  }
  def customParametersToMap()
  {
     [interpolation:interpolation, bands:bands,stretch_mode: stretch_mode, stretch_mode_region: stretch_mode_region, sharpen_mode:sharpen_mode,
             sharpen_width: sharpen_width as Double, sharpen_sigma: sharpen_sigma as Double, rotate: rotate as Double,
