...
OMAR.models.GeneralQueryModel = Backbone.Model.extend( {
    ...
    initialize: function ( params )
    {
        ...
        if ( params )
        {
            if ( params.niirsCheckbox )
            {
                this.attributes.niirsCheckbox = params.niirsCheckbox;
            }
            if ( params.niirsMinValue )
            {
                this.attributes.niirsMinValue = params.niirsMinValue;
            }
            if ( params.niirsMinValue )
            {
                this.attributes.niirsMinValue = params.niirsMinValue;
            ...
            if ( params.azimuthAngleCheckbox )
            {
                this.attributes.azimuthAngleCheckbox = params.azimuthAngleCheckbox;
            }
            if ( params.azimuthAngleMinValue )
            {
                this.attributes.azimuthAngleMinValue = params.azimuthAngleMinValue;
            }
            if ( params.azimuthAngleMaxValue )
            {
                this.attributes.azimuthAngleMaxValue = params.azimuthAngleMaxValue;
            }

            if ( params.grazingAngleCheckbox )
            {
                this.attributes.grazingAngleCheckbox = params.grazingAngleCheckbox;
            }
            if ( params.grazingAngleMinValue )
            {
                this.attributes.grazingAngleMinValue = params.grazingAngleMinValue;
            }
            if ( params.grazingAngleMaxValue )
            {
                this.attributes.grazingAngleMaxValue = params.grazingAngleMaxValue;
            }

            if ( params.sunAzimuthCheckbox )
            {
                this.attributes.sunAzimuthCheckbox = params.sunAzimuthCheckbox;
            }
            if ( params.sunAzimuthMinValue )
            {
                this.attributes.sunAzimuthMinValue = params.sunAzimuthMinValue;
            }
            if ( params.sunAzimuthMaxValue )
            {
                this.attributes.sunAzimuthMaxValue = params.sunAzimuthMaxValue;
            }

            if ( params.sunElevationCheckbox )
            {
                this.attributes.sunElevationCheckbox = params.sunElevationCheckbox;
            }
            if ( params.sunElevationMinValue )
            {
                this.attributes.sunElevationMinValue = params.sunElevationMinValue;
            }
            if ( params.sunElevationMaxValue )
            {
                this.attributes.sunElevationMaxValue = params.sunElevationMaxValue;
            }

            if ( params.cloudCoverageCheckbox )
            {
                this.attributes.cloudCoverageCheckbox = params.cloudCoverageCheckbox;
            }
            if ( params.cloudCoverageMaxValue )
            {
                this.attributes.cloudCoverageMaxValue = params.cloudCoverageMaxValue;
            }

            if ( params.missionCheckbox )
            {
                this.attributes.missionCheckbox = params.missionCheckbox;
            }
            if ( params.missionValue )
            {
                this.attributes.missionValue = params.missionValue;
            }
            if ( params.missionComparator )
            {
                this.attributes.missionComparator = params.missionComparator;
            }

            if ( params.sensorCheckbox )
            {
                this.attributes.sensorCheckbox = params.sensorCheckbox;
            }
            if ( params.sensorValue )
            {
                this.attributes.sensorValue = params.sensorValue;
            }
            if ( params.sensorComparator )
            {
                this.attributes.sensorComparator = params.sensorValue;
            }

            if ( params.beNumberCheckbox )
            {
                this.attributes.beNumberCheckbox = params.beNumberCheckbox;
            }
            if ( params.beNumberValue )
            {
                this.attributes.beNumberValue = params.beNumberValue;
            }
            if ( params.beNumberComparator )
            {
                this.attributes.beNumberComparator = params.beNumberComparator;
            }

            if ( params.targetCheckbox )
            {
                this.attributes.targetCheckbox = params.targetCheckbox;
            }
            if ( params.targetValue )
            {
                this.attributes.targetValue = params.targetValue;
            }
            if ( params.targetComparator )
            {
                this.attributes.targetComparator = params.targetComparator;
            }

            if ( params.wacCheckbox )
            {
                this.attributes.wacCheckbox = params.wacCheckbox;
            }
            if ( params.wacValue )
            {
                this.attributes.wacValue = params.wacValue;
            }
            if ( params.wacComparator )
            {
                this.attributes.wacComparator = params.wacComparator;
            }

            if ( params.filenameCheckbox )
            {
                this.attributes.filenameCheckbox = params.filenameCheckbox;
            }
            if ( params.filenameValue )
            {
                this.attributes.filenameValue = params.filenameValue;
            }
            if ( params.filenameComparator )
            {
                this.attributes.filenameComparator = params.filenameComparator;
            }

            if ( params.imageIdCheckbox )
            {
                this.attributes.imageIdCheckbox = params.imageIdCheckbox;
            }
            if ( params.imageIdValue )
            {
                this.attributes.imageIdValue = params.imageIdValue;
            }
            if ( params.imageIdComparator )
            {
                this.attributes.imageIdComparator = params.imageIdComparator;
            }
        ...
        $( dbAttributes ).each( function ( idx, obj )
        {
            ...
            if ( checked )
            {
                if ( value != null )
                {
                    ...
                    // SCOTTIE
                    // Special handling for BE Nums
                    if ( dbname === "be_number" && OMAR.beSearchParams.tableName )
                    {
                        var foo = temp.replace( 'be_number', OMAR.beSearchParams.columnName );

                        foo = foo.split( "'" ).join( "''" )
                        temp = "(" + temp + " or intersects(ground_geom, collectGeometries(queryCollection('" + OMAR.beSearchParams.tableName + "', '" + OMAR.beSearchParams.geomName + "', '" + foo + "'))))";
                    }
