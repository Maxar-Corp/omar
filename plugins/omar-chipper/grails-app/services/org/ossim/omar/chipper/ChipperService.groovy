...
//    def numBands = chipperOptionsMap?.bands?.split(',')?.size() ?: 3
    def outputSupportsTransparent = !( outputFormat in ['jpg', 'jpeg'] )
    def numBands = outputSupportsTransparent ? 4 : 3
...
/*
  ...
  */
  ...
  def getThumbnail(ThumbnailCommand cmd)
  {
    ...
    def cacheFile = new File( cacheDir, "ortho-${cmd.id}-${cmd.size}.${cmd.type}" )
    ...
    if ( !cacheFile.exists() )
    {
      def opts = [
          ...
          'image0.file': rasterEntry.filename,
          'image0.entry': rasterEntry.entryId,

          operation: 'ortho',
          output_file: cacheFile.absolutePath,
          output_radiometry: 'U8',
          pad_thumbnail: 'true',
          srs: 'epsg:4326',
          three_band_out: 'true',
          thumbnail_resolution: String.valueOf( cmd.size )
      ]
    ...
    [contentType: "image/${cmd.type}", content: cacheFile.bytes]
  }

  def __getThumbnail(ThumbnailCommand cmd)
  {
    def rasterEntry = RasterEntry.read( cmd.id )
    def type = cmd.type
    def ostream = new ByteArrayOutputStream()
