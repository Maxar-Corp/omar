        ...
        <div class="dialog">
            <table>
                ...
                <tr class="prop">
                    ...
                    <td valign="top"
                        class="value">${fieldValue( bean: rasterEntry, field: 'securityClassification' )}</td>
                </tr>
                <tr class="prop">
                ...
                <tr class="prop">
                    ...
                    <td valign="top" style="text-align:left;" class="value">
                        <g:if test="${rasterEntry.fileObjects}">
                            <g:link controller="rasterEntryFile" action="list"
                                    params="${[rasterEntryId: rasterEntry.id]}">Show Raster Files</g:link>
                        </g:if>
                    </td>
                </tr>
                <tr class="prop">
                    <td valign="top" class="name">Raster Data Set:</td>
                    <td valign="top" class="value"><g:link controller="rasterDataSet" action="show"
                                                           id="${rasterEntry?.rasterDataSet?.id}">${rasterEntry?.rasterDataSet?.encodeAsHTML()}</g:link></td>
                </tr>
                </tbody>
            </table>
            <table>
                ...
                <tr>
                    <th>Link</th>
                    <g:each in="${placemarkColumnNames}" var="columnName">
                        <th>${columnName}</th>
                    </g:each>
                </tr>
                </thead>
                <tbody>
                <g:each in="${placemarks}" var="placemark">
                    <tr>
                        <td><g:link controller="mapView" params="${[
                                layers: rasterEntry?.indexId,
                                longitude: placemark.geom.x,
                                latitude: placemark.geom.y,
                                fullres: true
                        ]}">GO THERE</g:link></td>

                        <g:each in="${placemarkColumnNames}" var="columnName">
                            <td>${placemark[columnName]}</td>
                        </g:each>
                    </tr>
                </g:each>
                </tbody>
