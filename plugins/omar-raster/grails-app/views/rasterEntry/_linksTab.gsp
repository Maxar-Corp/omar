      ...
      <tr class="${( i % 2 ) == 0 ? 'odd' : 'even'}">
        <td height="${rasterEntry.height / ( Math.max( rasterEntry.width, rasterEntry.height ) / 128.0 )}">
          <a href="${createLink( controller: "mapView", params: [layers: rasterEntry.indexId], absolute: true )}">
            <img src="${createLink( controller: "thumbnail", action: "show", params:
                [id: rasterEntry.id, size: 128, projectionType: "imagespace"], absolute: true )}" alt="Show Thumbnail"/>
          </a>
        </td>
        <td>
        ...
        <td>
          <a href="${createLink( controller: "ogc", action: "wms", params: [request: "GetCapabilities",
              layers: rasterEntry.indexId], absolute: true )}">WMS GetCapabilities</a>
        </td>
        <td>
          ...
          <a href="${createLink( controller: "ogc", action: "wms", params: [request: "GetMap",
              ...
              rasterEntry?.groundGeom?.bounds?.maxLat].join( "," ), srs: "epsg:4326", width: 1024, height: "${(Math.rint(rasterEntry.width / rasterEntry.height * 1024) as int)}",
              format: "image/jpeg"], absolute: true )}">WMS GetMap</a>
        </td>
        <td>
          <a href="${createLink( controller: "ogc", action: "wms", params: [request: "GetKML",
              layers: rasterEntry.indexId, format: "image/png", transparent: "true"], absolute: true )}">Generate KML</a>
        </td>
        <td>
          <a href="${createLink( controller: "superOverlay", action: "createKml", params: [id: rasterEntry.indexId,
              stretch_mode: "linear_auto_min_max", stretch_mode_region: "global"], absolute: true )}">Generate Super Overlay</a>
        </td>
      </tr>
    </g:each>
