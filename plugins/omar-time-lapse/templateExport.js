...
var logoOffset;
var logoWidth;
var arrowType = new Array();
var arrowHeight = new Array();
var arrowWidth = new Array();
var arrowHeightPosition = new Array();
var arrowWidthPosition = new Array();
...
function addAnArrow(typeOfArrow)
{
	var arrow = typeOfArrow;
	
	$("#pageContainer").append(
		'<div id = "arrowDiv"> \
			<img id = "arrowImage" src = ' + $("#logoImagesDirectory").get(0).innerHTML + 'annotationArrow.png height = "500"/> \
		</div>');

	var arrowImageId =  "arrowImage" + arrowType.length;
	$("#arrowImage").get(0).id = arrowImageId;
	$("#" + arrowImageId).resizable();
	
	var arrowDivId = "arrowDiv" + arrowType.length;
	$("#arrowDiv").get(0).id = arrowDivId;
	$("#" + arrowDivId).draggable();

	$("#" + arrowDivId).css("height", $("#" + arrowImageId).height());
	$("#" + arrowDivId).css("width", $("#" + arrowImageId).width());
	$("#" + arrowDivId).position({
                my: "left bottom",
                at: "left bottom",
                of: $("#previewImage")
...
function downloadImage()
{
	...
	if (arrowType.length > 0)
	{
		var annotationArrowOffsetHeight = new Array();
		var annotationArrowOffsetWidth = new Array();
		for (var i = 0; i < arrowType.length; i++)
		{
			annotationArrowOffsetHeight[i] = parseFloat($("#previewImage").offset.top) - parseFloat($("#arrowDiv" + i).offset().top);
			annotationArrowOffsetWidth[i] = parseFloat($("#previewImage").offset.left) - parseFloat($("#arrowDiv" + i).offset().left);	
		}
		exportUrlParams += "&annotationArrowOffsetHeight=" + annotationArrowOffsetHeight.join(",");
		exportUrlParams += "&annotationArrowOffsetWidth=" + annotationArrowOffsetWidth.join(",");
		alert(exportUrlParams);
	}
