...
def removeVideoFiles()
{
   def query;
   if(parent.scriptArgs.size())
   {
   query = "SELECT name FROM video_file WHERE "
   ...
   try{
    this.sql?.eachRow(query) { row ->
      ...
      def result = parent.postDataManager(file, "removeVideo");
...
{
   def dir = new File(parent.env.OMAR_THUMBNAIL_CACHE) 
   dir.traverse( type:FileType.FILES, nameFilter:~/video.*jpg/ ) 
   { file->
     def filenameOnly =  file.name;
     def splitDash = filenameOnly.split("-");

     try{
       def row = this.sql?.firstRow("SELECT id FROM video_data_set where id=${splitDash[1] as Integer}");
